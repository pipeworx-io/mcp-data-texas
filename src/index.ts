...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Texas Open Data');
}

const BASE = 'https://data.texas.gov';
const UA = 'pipeworx-mcp-data-texas/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Texas Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Texas Open Data dataset by resource_id (e.g. "54pj-3dxy"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "54pj-3dxy" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Texas Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "54pj-3dxy".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "54pj-3dxy".' } },
      required: ['resource_id'],
    },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"54pj-3dxy"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"54pj-3dxy"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'data.texas.gov');
  return res.json();
}
